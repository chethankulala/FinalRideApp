        ...
        Coroutines.main {
            try {
                ...
                dealersResponse.data?.let {
                    ...
                    val dealer = it
                    dealerId.value = id
                    dealerName.value = dealer.get("name").toString().replace("\"", "")
                    dealerAddress.value = dealer.get("address").toString().replace("\"","")+", "+dealer.get("city").toString().replace("\"", "")+", "+dealer.get("state").toString().replace("\"", "")
                    ...
                    BookServiceObject.updateChosenDealerDetails(dealer.get("name").toString().replace("\"", ""),
                        dealer.get("city").toString().replace("\"", ""),
